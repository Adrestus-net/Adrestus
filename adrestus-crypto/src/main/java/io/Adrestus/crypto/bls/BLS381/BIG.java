...
public class BIG implements Serializable {
    ...
    public String toRawString() {
        ...
        String s = "(";
        for (int i = 0; i < NLEN - 1; i++) {
            s += Long.toHexString(b.w[i]);
            s += ",";
        }
        s += Long.toHexString(b.w[NLEN - 1]);
        s += ")";
        return s;
    }
    ...
    public static BIG mul2(BIG a, BIG b) {
        BIG c = new BIG(0);
        a.norm();
        b.norm();
        long carry;
        long[] cr = new long[2];
        c.w = new long[BIG.DNLEN];
        for (int i = 0; i < NLEN; i++) {
            carry = 0;
            for (int j = 0; j < NLEN; j++) {
                cr = muladd(a.w[i], b.w[j], carry, c.w[i + j]);
    ...
    public static BIG frombytearrayUnlimited(byte[] b, int n) {
        BIG m = new BIG(0);
    ...
    public static BIG fromBytesUnlimited(byte[] b) {
        return frombytearrayUnlimited(b, 0);
    }
...
// i=4
    ...
    public boolean equals(Object obj) {
        ...
        BIG big = (BIG) obj;
        return Arrays.equals(this.w, big.w);
    }
}

package io.Adrestus.core.dcoker;
...
public class ConsensusDockerTest {
    ...
    private static final String KafkaDisperseByzantineTransactionBlockTestName = "KafkaDisperseByzantineTransactionBlockTest";
    private static final String KafkaDisperseTransactionBlockTestName = "KafkaDisperseTransactionBlockTest";
...
//    private static final String imageConsensusName = "dockerfile-consensus-tests";
    ...
    @Test
    public void KafkaDisperseByzantineTransactionBlockTest() throws IOException {
...
//                       "sh", "-c", "mvn clean install -DskipTests && mvn test -Dtest=" + VRFTestName +
                        "sh", "-c", "mvn test -Dtest=" + KafkaDisperseByzantineTransactionBlockTestName +
...
//                            System.out.println(logLine);
                            if (logLine.contains(KafkaDisperseByzantineTransactionBlockTestName)) {
                                first = true;
                            }
                            if (first) {
    ...
    @Test
    public void KafkaDisperseTransactionBlockTest() throws IOException {
        if (RunningConfig.isRunningInAppveyor()) {
            return;
        }
        ...
        for (int i = 0; i < ipv4Addresses.size(); i++) {
            ...
            try {
                String[] buildCommand = {
                        "sh", "-c", "mvn test -Dtest=" + KafkaDisperseTransactionBlockTestName +
                        " -Dtest.arg0=" + ipv4Addresses.get(0) +
                        " -Dtest.arg1=" + ipv4Addresses.get(1) +
                        " -Dtest.arg2=" + ipv4Addresses.get(2)
                };
            ...
            Thread.ofVirtual().start(() -> {
                try {
                    val.exec(new ResultCallback.Adapter<>() {
                        ...
                        public void onNext(Frame item) {
                            String logLine = new String(item.getPayload());
                            if (logLine.contains(KafkaDisperseTransactionBlockTestName)) {
                                first = true;
                            }
                            if (first) {
                                System.out.println(val.getContainerId() + " " + logLine);

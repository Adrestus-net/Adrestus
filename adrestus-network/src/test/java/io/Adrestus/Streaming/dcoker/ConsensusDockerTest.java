package io.Adrestus.Streaming.dcoker;
...
public class ConsensusDockerTest {
    ...
    private static final String KafkaMultiNodeFullStartStopTestName = "KafkaMultiNodeFullStartStopTest";
    private static final String KafkaMultiNodeFullTest = "KafkaMultiNodeFullTest";
    private static final String KafkaMultiNodeTest = "KafkaMultiNodeTest";
    private static final String KafkaMultiNodeTest2 = "KafkaMultiNodeTest2";
...
//                .exec();
    ...
    //@Test
    public void KafkaMultiNodeFullStartStopTest() throws IOException {
...
//                       "sh", "-c", "mvn clean install -DskipTests && mvn test -Dtest=" + VRFTestName +
                        "sh", "-c", "mvn test -Dtest=" + KafkaMultiNodeFullStartStopTestName +
...
//                            System.out.println(logLine);
                            if (logLine.contains(KafkaMultiNodeFullStartStopTestName)) {
                                first = true;
                            }
                            if (first) {
    ...
    //@Test
    public void KafkaMultiNodeFullTest() throws IOException {
        if (RunningConfig.isRunningInAppveyor()) {
            return;
        }
        ...
        for (int i = 0; i < ipv4Addresses.size(); i++) {
            ...
            try {
                String[] buildCommand = {
                        "sh", "-c", "mvn test -Dtest=" + KafkaMultiNodeFullTest +
                        " -Dtest.arg0=" + ipv4Addresses.get(0) +
                        " -Dtest.arg1=" + ipv4Addresses.get(1) +
                        " -Dtest.arg2=" + ipv4Addresses.get(2)
                };
            ...
            Thread.ofVirtual().start(() -> {
                try {
                    val.exec(new ResultCallback.Adapter<>() {
                        ...
                        public void onNext(Frame item) {
                            String logLine = new String(item.getPayload());
                            if (logLine.contains(KafkaMultiNodeFullTest)) {
                                first = true;
                            }
                            if (first) {
                                System.out.println(val.getContainerId() + " " + logLine);
    ...
    @Test
    public void KafkaMultiNodeTest() throws IOException {
        if (RunningConfig.isRunningInAppveyor()) {
            return;
        }
        ...
        for (int i = 0; i < ipv4Addresses.size(); i++) {
            ...
            try {
                String[] buildCommand = {
                        "sh", "-c", "mvn test -Dtest=" + KafkaMultiNodeTest +
                        " -Dtest.arg0=" + ipv4Addresses.get(0) +
                        " -Dtest.arg1=" + ipv4Addresses.get(1) +
                        " -Dtest.arg2=" + ipv4Addresses.get(2)
                };
                ...
                try {
                    val.exec(new ResultCallback.Adapter<>() {
                        ...
                        public void onNext(Frame item) {
                            String logLine = new String(item.getPayload());
                            //System.out.println(logLine);
                            if (logLine.contains(KafkaMultiNodeTest)) {
                                first = true;
                            }
                            if (first) {
                                System.out.println(val.getContainerId() + " " + logLine);
    ...
    @Test
    public void KafkaMultiNodeTest2() throws IOException {
        if (RunningConfig.isRunningInAppveyor()) {
            return;
        }
        ...
        for (int i = 0; i < ipv4Addresses.size(); i++) {
            ...
            try {
                String[] buildCommand = {
                        "sh", "-c", "mvn test -Dtest=" + KafkaMultiNodeTest2 +
                        " -Dtest.arg0=" + ipv4Addresses.get(0) +
                        " -Dtest.arg1=" + ipv4Addresses.get(1) +
                        " -Dtest.arg2=" + ipv4Addresses.get(2)
                };
                ...
                try {
                    val.exec(new ResultCallback.Adapter<>() {
                        ...
                        public void onNext(Frame item) {
                            String logLine = new String(item.getPayload());
                            //System.out.println(logLine);
                            if (logLine.contains(KafkaMultiNodeTest2)) {
                                first = true;
                            }
                            if (first) {
                                System.out.println(val.getContainerId() + " " + logLine);

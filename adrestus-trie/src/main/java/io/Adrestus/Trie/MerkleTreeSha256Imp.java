...
public class MerkleTreeSha256Imp implements MerkleTree {
    ...
    public void constructTree(List<MerkleNode> dataBlocks) {
        ...
        if (dataBlocks.size() == 1) {
            ...
            String left = HashUtil.sha256(dataBlocks.get(0).getTransactionHash());
            String right = HashUtil.sha256(dataBlocks.get(0).getTransactionHash());
            String hash = HashUtil.sha256(left + right);
            ...
            String left = HashUtil.sha256(dataBlocks.get(0).getTransactionHash());
            String right = HashUtil.sha256(dataBlocks.get(1).getTransactionHash());
            String hash = HashUtil.sha256(left + right);
            MerkleNode leftMerkleNode = new MerkleNode(left);
            MerkleNode rightMerkleNode = new MerkleNode(right);
            leftMerkleNode.setRoot(this.root);
        ...
        for (int i = 0; i < dataBlocks.size(); i++) {
            ...
            node.setTransactionHash(HashUtil.sha256(node.getTransactionHash()));
    ...
    public RecursiveOptimizer<Integer> ConstructMap(int iterate) {
        ...
        for (int i = 0; i < this.merkleNodeHashMapByIndex.get(iterate).size(); i += 2) {
            ...
            String hash = HashUtil.sha256(left.getTransactionHash() + right.getTransactionHash());
    ...
    public String generateRoot(MerkleProofs proofs) {
        ...
        String hash = HashUtil.sha256(proofs.getProofs().get(new ProfKey(LOWEST_INDEX_LEFT, true)).getTransactionHash() + proofs.getProofs().get(new ProfKey(LOWEST_INDEX_RIGHT, false)).getTransactionHash());
        ...
        for (Map.Entry<ProfKey, MerkleNode> entry : proofs.getProofs().entrySet()) {
            ...
            if (key.isLeft()) {
                hash = HashUtil.sha256(node.getTransactionHash() + hash);
            } else {
                hash = HashUtil.sha256(hash + node.getTransactionHash());
            }
        }
        return hash;
    }
